...
- Mod installer: extend **OdyPatch** host and **OdyPatch.UI** library (not HoloPatcher); patch engine in `BioWare.TSLPatcher`. Run/test via `OdyPatch.csproj`. Validate without game install via `--validate` fixture; manual E2E via [odypatch-e2e-runbook.md](odypatch-e2e-runbook.md). `[REPO]`
