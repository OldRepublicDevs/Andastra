...
- NCS bytecode scanning **enabled** — StrRef CONSTI via `ReferenceCache` (plan **286**); script ResRef via `NcsConstStringScanner` (plan **287**). CONSTI disambiguation + control-flow arc **landed** on PR #36 (plans **292**–**341**): threshold, opcode-context, action-signature, relay hops, jump-fork CF scan, cache-path gating, slow vs cache docs; **86** NcsConsti tests. JSR call-literal StrRef consumer closed PR #46 (plan **356**). JSR multi-arg slot alignment closed PR #48 (plan **358**). Nested JSR relay StrRef closed PR #50 (plan **360**). Nested JSR multi-arg relay StrRef closed PR #52 (plan **362**). Full stack simulation remains deferred.
