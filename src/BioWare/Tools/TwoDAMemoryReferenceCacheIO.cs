...
{
    /// <summary>
    /// JSON persistence for <see cref="TwoDAMemoryReferenceCache"/> (KotorCLI find-2da-ref --cache-file).
    /// </summary>
    public static class TwoDAMemoryReferenceCacheIO
    {
        private sealed class TwoDACacheFilePayload
        {
            public string Game { get; set; }
            public Dictionary<string, object> CacheData { get; set; }
        }

        public static void Save(string cacheFilePath, TwoDAMemoryReferenceCache cache)
        {
            ...
            var payload = new TwoDACacheFilePayload
            {
        ...
        public static TwoDAMemoryReferenceCache Load(string cacheFilePath)
        {
            return Load(cacheFilePath, BioWareGame.K1, validateGame: false);
        }

        public static TwoDAMemoryReferenceCache Load(string cacheFilePath, BioWareGame expectedGame, bool validateGame)
        {
            if (string.IsNullOrWhiteSpace(cacheFilePath))
            {
                throw new ArgumentException("Cache file path is required.", nameof(cacheFilePath));
            ...
            {
                throw new FileNotFoundException("2DA memory cache file not found.", cacheFilePath);
            }

            TwoDACacheFilePayload payload = JsonConvert.DeserializeObject<TwoDACacheFilePayload>(File.ReadAllText(cacheFilePath));
            if (payload == null || payload.CacheData == null)
            {
                throw new InvalidDataException("2DA memory cache file is empty or invalid.");
            }
            ...
            if (validateGame && expectedGame != game)
            {
                throw new InvalidDataException(
                    "2DA memory cache game mismatch: file is " + payload.Game + " but installation is " + FormatGame(expectedGame) + ".");
            }

            Dictionary<string, List<Dictionary<string, object>>> typedData = ConvertFromObjectDict(payload.CacheData);
            return TwoDAMemoryReferenceCache.FromDict(game, typedData);
        }
