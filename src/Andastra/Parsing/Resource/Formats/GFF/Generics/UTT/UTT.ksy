meta:
  ...
  xref:
    ...
    xoreos: vendor/xoreos/src/aurora/uttfile.cpp
    wiki: vendor/PyKotor/wiki/GFF-File-Format.md
doc: |
  UTT (Trigger Template) files are GFF-based format files that store trigger definitions including
  geometry, scripts, trap settings, and trigger behavior. UTT files use the GFF (Generic File Format)
  binary structure with file type signature "UTT ".
  
  UTT files contain:
  - Root struct with trigger metadata:
    - TemplateResRef (ResRef): Resource reference for this trigger template
    - Tag (String): Tag identifier for this trigger
    - LocalizedName (LocalizedString): Localized name (not used by game engine)
    - KeyName (String): Tag of the key item required to activate trigger
    - Type (UInt32): Trigger type identifier
    - TrapFlag (Byte): Whether trigger has a trap (0 = no trap, 1 = trap)
    - OnTrapTriggered (ResRef): Script to run when trap triggers
    - OnClick (ResRef): Script to run when trigger is clicked
    - ScriptOnEnter (ResRef): Script to run when area is entered
    - ScriptOnExit (ResRef): Script to run when area is exited
    - Comment (String): Developer comment
  
  Trigger types (Type field):
  - 0 = Generic
  - 1 = Waypoint
  - 2 = Door
  - 3 = Placeable
  ...
  References:
  - vendor/PyKotor/Libraries/PyKotor/src/pykotor/resource/generics/utt.py:16-255
  - vendor/reone/src/libs/resource/parser/gff/utt.cpp
  - vendor/xoreos/src/aurora/uttfile.cpp
    ...
    seq:
      - id: labels
        ...
        doc: |
          ...
          - "KeyName" (String field in root struct)
          - "Type" (UInt32 field in root struct)
          - "TrapFlag" (Byte field in root struct)
          - "OnTrapTriggered" (ResRef field in root struct)
          - "OnClick" (ResRef field in root struct)
          - "ScriptOnEnter" (ResRef field in root struct)
          - "ScriptOnExit" (ResRef field in root struct)
    ...
    seq:
      - id: field_type
        ...
        doc: |
          ...
          UTT-specific field types:
          - TemplateResRef: field_type = 11 (ResRef)
          - Tag: field_type = 10 (String)
          - LocalizedName: field_type = 12 (LocalizedString)
          - KeyName: field_type = 10 (String)
          - Type: field_type = 4 (UInt32)
          - TrapFlag: field_type = 0 (Byte)
          - OnTrapTriggered: field_type = 11 (ResRef)
          - OnClick: field_type = 11 (ResRef)
          - ScriptOnEnter: field_type = 11 (ResRef)
          - ScriptOnExit: field_type = 11 (ResRef)
    ...
    seq:
      - id: data
        ...
        doc: |
          ...
          UTT files use field_data for:
          - TemplateResRef (ResRef): 1-byte length + up to 16 bytes ASCII
          - Tag (String): 4-byte length + string bytes
          - LocalizedName (LocalizedString): Variable size (see LocalizedString structure)
          - KeyName (String): 4-byte length + string bytes
          - OnTrapTriggered (ResRef): 1-byte length + up to 16 bytes ASCII
          - OnClick (ResRef): 1-byte length + up to 16 bytes ASCII
          - ScriptOnEnter (ResRef): 1-byte length + up to 16 bytes ASCII
          - ScriptOnExit (ResRef): 1-byte length + up to 16 bytes ASCII
          - Comment (String): 4-byte length + string bytes
    ...
    seq:
      - id: indices
        ...
        doc: |
          ...
          UTT files typically do not use LIST fields in the root struct, but the
          structure is available for future extensions or nested data.
