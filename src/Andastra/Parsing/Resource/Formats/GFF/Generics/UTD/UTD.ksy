...
doc: |
  UTD (Door) files are GFF-based format files that store door definitions including
  lock/unlock mechanics, HP, scripts, and appearance. UTD files use the GFF (Generic File Format)
  binary structure with file type signature "UTD ".
  
  UTD files contain:
  - Root struct with door metadata (Tag, LocName, TemplateResRef, etc.)
  - Lock/unlock properties (Lockable, Locked, KeyRequired, OpenLockDC, etc.)
  - HP and combat stats (HP, CurrentHP, Hardness, Fort, Ref, Will)
  - Appearance and animation (GenericType, AnimationState, PaletteID)
  - Script hooks (OnClick, OnOpen, OnClose, OnDeath, OnHeartbeat, etc.)
  - KotOR 2 specific fields (OpenLockDiff, OpenLockDiffMod, OpenState, NotBlastable, Min1HP)
  - Deprecated trap properties (TrapDetectable, TrapDisarmable, DisarmDC, etc.)
  
  Each field in the root struct contains:
  - Basic door properties (Tag, LocName, TemplateResRef)
  - Lock mechanics (Lockable, Locked, KeyRequired, OpenLockDC)
  - HP and stats (HP, CurrentHP, Hardness, Fortitude, Reflex, Willpower)
  - Script references (ResRef) for various events
  - Appearance settings (GenericType, AnimationState, PaletteID)
  ...
  - vendor/xoreos/src/aurora/gfffile.cpp:166-226
  ...
  gff_header:
    seq:
      - id: file_type
        ...
        size: 4
        doc: |
          File type signature. Must be "UTD " for door files.
          Other GFF types: "GFF ", "ARE ", "DLG ", "UTC ", "UTI ", etc.
        valid: "UTD "
