...
namespace Andastra.Game.Scripting
{
    ...
    ///   - k2_win_gog_aspyr_swkotor2.exe: DispatchScriptEvent @ 0x004dd730 - Dispatches script events to registered handlers, creates event data structure, iterates through registered script handlers, calls 0x004db870 to match event types, queues matching handlers
    ///   - k2_win_gog_aspyr_swkotor2.exe: LoadScriptHooks @ 0x0050c510 - Loads script hook references from GFF templates (ScriptHeartbeat, ScriptOnNotice, ScriptSpellAt, ScriptAttacked, ScriptDamaged, ScriptDisturbed, ScriptEndRound, ScriptDialogue, ScriptSpawn, ScriptRested, ScriptDeath, ScriptUserDefine, ScriptOnBlocked, ScriptEndDialogue)
    ///   - k2_win_gog_aspyr_swkotor2.exe: LogScriptEvent @ 0x004dcfb0 - Logs script events for debugging, maps event types to string names
    ///   - k1_win_gog_swkotor.exe: DispatchScriptEvent (0x004af630 @ 0x004af630) - Equivalent to k2_win_gog_aspyr_swkotor2.exe DispatchScriptEvent, dispatches script events to registered handlers, creates event data structure, iterates through registered script handlers, matches event types, queues matching handlers. Located via string references: "CSWSSCRIPTEVENT_EVENTTYPE_ON_HEARTBEAT" @ 0x00744958, "CSWSSCRIPTEVENT_EVENTTYPE_ON_PERCEPTION" @ 0x00744930, "CSWSSCRIPTEVENT_EVENTTYPE_ON_SPELLCASTAT" @ 0x00744904 (k1_win_gog_swkotor.exe). Event subtype mapping logic is identical to k2_win_gog_aspyr_swkotor2.exe but uses different string constant addresses.
    ///   - k1_win_gog_swkotor.exe: LoadScriptHooks - Loads script hook references from GFF templates (ScriptHeartbeat, ScriptOnNotice, ScriptSpellAt, ScriptAttacked, ScriptDamaged, ScriptDisturbed, ScriptEndRound, ScriptDialogue, ScriptSpawn, ScriptRested, ScriptDeath, ScriptUserDefine, ScriptOnBlocked, ScriptEndDialogue). Function address needs verification  analysis (expected in 0x004f-0x0050 address range based on k1_win_gog_swkotor.exe function address patterns).
    ///   - k1_win_gog_swkotor.exe: LogScriptEvent - Logs script events for debugging, maps event types to string names. String constants located at 0x00744958 (ON_HEARTBEAT), 0x00744930 (ON_PERCEPTION), 0x00744904 (ON_SPELLCASTAT), 0x007448dc (ON_DAMAGED), 0x007448b4 (ON_DISTURBED), 0x0074488c (ON_DIALOGUE), 0x00744864 (ON_SPAWN_IN), 0x00744840 (ON_RESTED), 0x0074481c (ON_DEATH), 0x007447ec (ON_USER_DEFINED_EVENT). Function address needs verification  analysis (may be integrated into 0x004af630 or separate function).
    ...
    public class ScriptExecutor : IScriptExecutor
    {
        ...
        /// - K1: ExecuteCommandExecuteScript @ 0x00535b70 (k1_win_gog_swkotor.exe) - Command handler for ExecuteScript NWScript command, processes script ResRef and object ID from VM stack, calls CVirtualMachine::RunScript @ 0x005d45d0 for actual execution. Registered at command index 8 in CSWVirtualMachineCommands::InitializeCommands @ 0x0054c960.
        /// - TSL: Equivalent function address needs verification (expected similar pattern in k2_win_gog_aspyr_swkotor2.exe).
