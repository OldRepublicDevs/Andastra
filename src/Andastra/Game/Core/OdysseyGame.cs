...
namespace Andastra.Runtime.Game.Core
{
    ...
    public class OdysseyGame : IDisposable
    {
        ...
        private void Update(float deltaTime)
        {
            ...
            // Load gameplay options menu GUI if needed
            if (_currentState == GameState.GameplayOptionsMenu && !_gameplayOptionsMenuGuiLoaded && _guiManager != null)
            {
                // Load gameplay options menu GUI
                // Based on swkotor2.exe: CSWGuiOptionsMain::OnGameplayOpt @ 0x006de240 loads "optionsgameplay" GUI
                string guiName = "optionsgameplay"; // Gameplay options menu GUI file
                int viewportWidth = _graphicsDevice.Viewport.Width;
                int viewportHeight = _graphicsDevice.Viewport.Height;
                
                if (_guiManager.LoadGui(guiName, viewportWidth, viewportHeight))
                {
                    _gameplayOptionsMenuGuiLoaded = true;
                    Console.WriteLine($"[Odyssey] Gameplay options menu GUI loaded: {guiName}");
                }
                else
                {
                    // Fallback: GUI file might not exist, continue without GUI (menu will still work)
                    _gameplayOptionsMenuGuiLoaded = true; // Mark as loaded to avoid retrying
                    Console.WriteLine($"[Odyssey] Gameplay options menu GUI not found: {guiName}, using fallback rendering");
                }
            }
        ...
        #region Graphics Options Submenu

        /// <summary>
        /// Initializes the graphics options submenu.
        /// </summary>
        private void InitializeGraphicsOptionsMenu()
        {
            // Create graphics-specific options list
            // Based on swkotor2.exe: CSWGuiOptionsMain::OnGraphicsOpt @ 0x006e3d80
            // Graphics settings from GameSettings.Graphics
            _graphicsOptions = new List<Andastra.Runtime.Game.GUI.OptionsMenu.OptionItem>
            {
                new Andastra.Runtime.Game.GUI.OptionsMenu.OptionItem(
                    "Resolution Width",
                    Andastra.Runtime.Game.GUI.OptionsMenu.OptionType.Numeric,
                    () => _settings.Graphics.ResolutionWidth,
                    v => _settings.Graphics.ResolutionWidth = (int)v,
                    320, 7680),
                new Andastra.Runtime.Game.GUI.OptionsMenu.OptionItem(
                    "Resolution Height",
                    Andastra.Runtime.Game.GUI.OptionsMenu.OptionType.Numeric,
                    () => _settings.Graphics.ResolutionHeight,
                    v => _settings.Graphics.ResolutionHeight = (int)v,
                    240, 4320),
                new Andastra.Runtime.Game.GUI.OptionsMenu.OptionItem(
                    "Fullscreen",
                    Andastra.Runtime.Game.GUI.OptionsMenu.OptionType.Boolean,
                    () => _settings.Graphics.Fullscreen ? 1 : 0,
                    v => _settings.Graphics.Fullscreen = v > 0,
                    0, 1),
                new Andastra.Runtime.Game.GUI.OptionsMenu.OptionItem(
                    "VSync",
                    Andastra.Runtime.Game.GUI.OptionsMenu.OptionType.Boolean,
                    () => _settings.Graphics.VSync ? 1 : 0,
                    v => _settings.Graphics.VSync = v > 0,
                    0, 1),
                new Andastra.Runtime.Game.GUI.OptionsMenu.OptionItem(
                    "Texture Quality",
                    Andastra.Runtime.Game.GUI.OptionsMenu.OptionType.Numeric,
                    () => _settings.Graphics.TextureQuality,
                    v => _settings.Graphics.TextureQuality = (int)v,
                    0, 2),
                new Andastra.Runtime.Game.GUI.OptionsMenu.OptionItem(
                    "Shadow Quality",
                    Andastra.Runtime.Game.GUI.OptionsMenu.OptionType.Numeric,
                    () => _settings.Graphics.ShadowQuality,
                    v => _settings.Graphics.ShadowQuality = (int)v,
                    0, 3),
                new Andastra.Runtime.Game.GUI.OptionsMenu.OptionItem(
                    "Anisotropic Filtering",
                    Andastra.Runtime.Game.GUI.OptionsMenu.OptionType.Numeric,
                    () => _settings.Graphics.AnisotropicFiltering,
                    v =>
                    {
                        // Clamp to valid values: 0, 2, 4, 8, 16
                        int[] validValues = { 0, 2, 4, 8, 16 };
                        int targetValue = (int)v;
                        int closest = validValues[0];
                        int minDiff = Math.Abs(targetValue - closest);
                        foreach (int valid in validValues)
                        {
                            int diff = Math.Abs(targetValue - valid);
                            if (diff < minDiff)
                            {
                                minDiff = diff;
                                closest = valid;
                        ...
                        _settings.Graphics.AnisotropicFiltering = closest;
                    },
                    0, 16),
                new Andastra.Runtime.Game.GUI.OptionsMenu.OptionItem(
                    "Anti-Aliasing",
                    Andastra.Runtime.Game.GUI.OptionsMenu.OptionType.Boolean,
                    () => _settings.Graphics.AntiAliasing ? 1 : 0,
                    v => _settings.Graphics.AntiAliasing = v > 0,
                    0, 1)
            };
            _selectedGraphicsOptionIndex = 0;
            _isEditingGraphicsOptionValue = false;
            _editingGraphicsOptionValue = string.Empty;
            Console.WriteLine("[Odyssey] Graphics options submenu initialized");
        }

        /// <summary>
        /// Opens the graphics options submenu.
        /// </summary>
        private void OpenGraphicsOptionsMenu()
        {
            InitializeGraphicsOptionsMenu();
            _currentState = GameState.GraphicsOptionsMenu;
            Console.WriteLine("[Odyssey] Graphics options submenu opened");
        }

        /// <summary>
        /// Closes the graphics options submenu and returns to options menu.
        /// </summary>
        private void CloseGraphicsOptionsMenu()
        {
            _currentState = GameState.OptionsMenu;
            Console.WriteLine("[Odyssey] Graphics options submenu closed, returning to options menu");
        }

        /// <summary>
        /// Updates the graphics options submenu.
        /// </summary>
        private void UpdateGraphicsOptionsMenu(float deltaTime, IKeyboardState keyboardState, IMouseState mouseState)
        {
            var inputManager = _graphicsBackend.InputManager;
            var currentKeyboard = inputManager.KeyboardState;
            var currentMouse = inputManager.MouseState;

            if (_graphicsOptions == null || _graphicsOptions.Count == 0)
            {
                InitializeGraphicsOptionsMenu();
            }

            if (_isEditingGraphicsOptionValue)
            {
                // Handle text input for numeric values
                HandleGraphicsOptionValueInput(currentKeyboard, _previousKeyboardState, ref _editingGraphicsOptionValue, ref _isEditingGraphicsOptionValue);
                _previousKeyboardState = currentKeyboard;
            ...
            if (IsKeyJustPressed(_previousKeyboardState, currentKeyboard, Keys.Up))
            {
                _selectedGraphicsOptionIndex = Math.Max(0, _selectedGraphicsOptionIndex - 1);
            }
            if (IsKeyJustPressed(_previousKeyboardState, currentKeyboard, Keys.Down))
            {
                _selectedGraphicsOptionIndex = Math.Min(_graphicsOptions.Count - 1, _selectedGraphicsOptionIndex + 1);
            }

            // Option value modification (A/D or Left/Right)
            if ((IsKeyJustPressed(_previousKeyboardState, currentKeyboard, Keys.A) || IsKeyJustPressed(_previousKeyboardState, currentKeyboard, Keys.Left)) ||
                (IsKeyJustPressed(_previousKeyboardState, currentKeyboard, Keys.D) || IsKeyJustPressed(_previousKeyboardState, currentKeyboard, Keys.Right)))
            {
                if (_selectedGraphicsOptionIndex >= 0 && _selectedGraphicsOptionIndex < _graphicsOptions.Count)
                {
                    Andastra.Runtime.Game.GUI.OptionsMenu.OptionItem option = _graphicsOptions[_selectedGraphicsOptionIndex];
                    bool increase = IsKeyJustPressed(_previousKeyboardState, currentKeyboard, Keys.D) || IsKeyJustPressed(_previousKeyboardState, currentKeyboard, Keys.Right);
                    ModifyGraphicsOptionValue(option, increase);
                }
            }

            // Enter to edit numeric values
            if (IsKeyJustPressed(_previousKeyboardState, currentKeyboard, Keys.Enter) && 
                _selectedGraphicsOptionIndex >= 0 && _selectedGraphicsOptionIndex < _graphicsOptions.Count)
            {
                Andastra.Runtime.Game.GUI.OptionsMenu.OptionItem option = _graphicsOptions[_selectedGraphicsOptionIndex];
                if (option.Type == Andastra.Runtime.Game.GUI.OptionsMenu.OptionType.Numeric)
                {
                    _isEditingGraphicsOptionValue = true;
                    _editingGraphicsOptionValue = option.GetStringValue();
                }
            }
        ...
        /// <summary>
        /// Draws the graphics options submenu.
        /// </summary>
        private void DrawGraphicsOptionsMenu()
        {
            if (_spriteBatch == null || _font == null || _menuTexture == null || _graphicsOptions == null)
            {
                _graphicsDevice.Clear(new Color(20, 20, 30, 255));
                return;
            }
            ...
            int optionsStartY = 100;
            int optionWidth = viewportWidth - 100;
            int optionHeight = 50;
            int optionSpacing = 5;
            int maxVisible = (viewportHeight - optionsStartY - 100) / (optionHeight + optionSpacing);
            int startIdx = Math.Max(0, _selectedGraphicsOptionIndex - maxVisible / 2);
            int endIdx = Math.Min(_graphicsOptions.Count, startIdx + maxVisible);

            for (int i = startIdx; i < endIdx; i++)
            {
                int y = optionsStartY + (i - startIdx) * (optionHeight + optionSpacing);
                bool isSelected = (i == _selectedGraphicsOptionIndex);
                Color bgColor = isSelected ? new Color(100, 100, 150) : new Color(50, 50, 70);
                Rectangle optionRect = new Rectangle(optionsStartX, y, optionWidth, optionHeight);
                _spriteBatch.Draw(_menuTexture, optionRect, bgColor);

                Andastra.Runtime.Game.GUI.OptionsMenu.OptionItem option = _graphicsOptions[i];
                string optionText = option.Name + ": " + (_isEditingGraphicsOptionValue && i == _selectedGraphicsOptionIndex ? _editingGraphicsOptionValue + "_" : GetGraphicsOptionDisplayValue(option));
                Vector2 textPos = new Vector2(optionRect.X + 20, optionRect.Y + (optionHeight - _font.LineSpacing) / 2);
                _spriteBatch.DrawString(_font, optionText, textPos, new Color(255, 255, 255, 255));

                // Draw arrows for navigation hint
                if (isSelected && option.Type != Andastra.Runtime.Game.GUI.OptionsMenu.OptionType.Numeric)
                {
                    string arrows = "< A/D or Arrow Keys >";
                    Vector2 arrowsSize = _font.MeasureString(arrows);
                    Vector2 arrowsPos = new Vector2(optionRect.Right - arrowsSize.X - 20, optionRect.Y + (optionHeight - _font.LineSpacing) / 2);
                    _spriteBatch.DrawString(_font, arrows, arrowsPos, new Color(200, 200, 200, 255));
                }
            }

            // Instructions
            string instructions = _isEditingGraphicsOptionValue
                ? "Enter new value, then press Enter to confirm or Escape to cancel"
                : "Use Arrow Keys to navigate, A/D or Arrow Keys to change values, Enter to apply, Escape to cancel";
            Vector2 instSize = _font.MeasureString(instructions);
        ...
        /// <summary>
        /// Gets the display value for a graphics option.
        /// </summary>
        private string GetGraphicsOptionDisplayValue(Andastra.Runtime.Game.GUI.OptionsMenu.OptionItem option)
        {
            double value = option.GetValue();
            if (option.Type == Andastra.Runtime.Game.GUI.OptionsMenu.OptionType.Boolean)
            {
                return value > 0 ? "On" : "Off";
            }
            else if (option.Type == Andastra.Runtime.Game.GUI.OptionsMenu.OptionType.Numeric)
            {
                // Special formatting for specific options
                if (option.Name == "Texture Quality")
                {
                ...
                else if (option.Name == "Shadow Quality")
                {
                    string[] qualityNames = { "Off", "Low", "Medium", "High" };
                    int index = (int)value;
                    if (index >= 0 && index < qualityNames.Length)
                        return qualityNames[index];
                }
                else if (option.Name == "Anisotropic Filtering")
                {
                    int filterValue = (int)value;
                    if (filterValue == 0)
                        return "Off";
                    else
                        return filterValue + "x";
                }
        ...
        /// <summary>
        /// Modifies a graphics option value.
        /// </summary>
        private void ModifyGraphicsOptionValue(Andastra.Runtime.Game.GUI.OptionsMenu.OptionItem option, bool increase)
        {
            if (option.Type == Andastra.Runtime.Game.GUI.OptionsMenu.OptionType.Boolean)
            {
            ...
            else if (option.Type == Andastra.Runtime.Game.GUI.OptionsMenu.OptionType.Numeric)
            {
                ...
                // Special handling for Anisotropic Filtering (valid values: 0, 2, 4, 8, 16)
                if (option.Name == "Anisotropic Filtering")
                {
                    int[] validValues = { 0, 2, 4, 8, 16 };
                    int currentIndex = Array.IndexOf(validValues, (int)currentValue);
                    if (currentIndex == -1) currentIndex = 0;
                    int newIndex = increase ? Math.Min(validValues.Length - 1, currentIndex + 1) : Math.Max(0, currentIndex - 1);
                    option.SetValue(validValues[newIndex]);
                }
                else
                {
        ...
        /// <summary>
        /// Handles value input for numeric editing in graphics options.
        /// </summary>
        private void HandleGraphicsOptionValueInput(
        ...
        /// <summary>
        /// Applies graphics options settings.
        /// </summary>
        private void ApplyGraphicsOptions()
        {
            ...
            // Settings are already applied when SetValue is called
            // This method can be used for any additional processing needed
            Console.WriteLine("[Odyssey] Graphics options applied");
            Console.WriteLine($"[Odyssey] Resolution: {_settings.Graphics.ResolutionWidth}x{_settings.Graphics.ResolutionHeight}");
            Console.WriteLine($"[Odyssey] Fullscreen: {_settings.Graphics.Fullscreen}, VSync: {_settings.Graphics.VSync}");
            Console.WriteLine($"[Odyssey] Texture Quality: {_settings.Graphics.TextureQuality}, Shadow Quality: {_settings.Graphics.ShadowQuality}");
            Console.WriteLine($"[Odyssey] Anisotropic Filtering: {_settings.Graphics.AnisotropicFiltering}x, Anti-Aliasing: {_settings.Graphics.AntiAliasing}");

            // TODO: Apply resolution and fullscreen changes to graphics device when support is added
            // TODO: Apply VSync, texture quality, shadow quality, anisotropic filtering, anti-aliasing to renderer
        }
