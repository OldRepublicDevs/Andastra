...
{
    /// <summary>
    /// Odyssey engine (KOTOR 1 & 2) scene builder for Stride graphics backend.
    /// Builds rendering structures from LYT (layout) and VIS (visibility) files.
    /// </summary>
    /// <remarks>
    /// Odyssey Scene Builder (Stride):
    ...
    ///   - StrideOdysseySceneBuilder (this class) - Odyssey-specific LYT/VIS file handling for Stride backend
    ///   - MonoGame OdysseySceneBuilder (Runtime.MonoGame.Scene) - Same functionality for MonoGame backend
    /// </remarks>
    public class StrideOdysseySceneBuilder : BaseSceneBuilder
    {
        private readonly StrideGraphics.GraphicsDevice _graphicsDevice;
        private readonly IGameResourceProvider _resourceProvider;

        public StrideOdysseySceneBuilder([NotNull] StrideGraphics.GraphicsDevice device, [NotNull] IGameResourceProvider resourceProvider)
        {
            if (device == null)
            {
        ...
        protected override void BuildSceneInternal(object areaData)
        {
            ...
            Console.WriteLine("[StrideOdysseySceneBuilder] Building scene from LYT/VIS data");
            Console.WriteLine("[StrideOdysseySceneBuilder] Rooms: " + lyt.Rooms.Count);
            Console.WriteLine("[StrideOdysseySceneBuilder] Doorhooks: " + lyt.Doorhooks.Count);
            ...
            Console.WriteLine("[StrideOdysseySceneBuilder] Scene built with " + sceneData.Rooms.Count + " rooms");
        }
    ...
    /// <summary>
    /// Scene data structure holding room information and visibility graph (Stride-specific).
    /// </summary>
    ...
    /// <summary>
    /// Scene room data for rendering (Odyssey-specific, Stride backend).
    /// </summary>
