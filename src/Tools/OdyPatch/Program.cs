...
using OdyPatchCore = OdyPatch.UI.Core;
    ...
    class Program
    {
        ...
        private static void ExecuteCli(CommandLineArgs args)
        {
            ...
            if (string.IsNullOrEmpty(args.TslPatchData))
            {
                ...
                Environment.Exit((int)OdyPatch.UI.Core.ExitCode.NumberOfArgs);
                return;
            }
            ...
            try
            {
                gamePath = OdyPatch.UI.Core.ValidateGameDirectory(args.GameDir);
            }
            ...
            try
            {
                if (args.Install)
                {
                    ...
                    OdyPatch.UI.Core.InstallResult result = OdyPatch.UI.Core.InstallMod(
                        modInfo.ModPath,
                        gamePath,
                        modInfo.Namespaces,
                        selectedNamespace,
                    ...
                    Console.WriteLine($"[Info] Install time: {OdyPatch.UI.Core.FormatInstallTime(result.InstallTime)}");

                    if (result.NumErrors > 0)
                    {
                        Environment.Exit((int)OdyPatch.UI.Core.ExitCode.InstallCompletedWithErrors);
                    }
                ...
                else if (args.Uninstall)
                {
                    ...
                    bool fullyRan = OdyPatch.UI.Core.UninstallMod(modInfo.ModPath, gamePath, logger);
                    if (fullyRan)
                    {
            ...
            catch (Exception ex)
            {
                ...
                Environment.Exit((int)OdyPatch.UI.Core.ExitCode.ExceptionDuringInstall);
            }
        }
        ...
        public static AppBuilder BuildAvaloniaApp()
            => AppBuilder.Configure<OdyPatch.UI.App>()
                .UsePlatformDetect()
                //.WithInterFont()
                .LogToTrace();
    }
