...
namespace OdyPatch.UI.Update
{
    ...
    public sealed class AutoUpdater
    {
        ...
        private async Task<string> DownloadUpdateAsync(string tempRoot, IUpdateProgress progress, CancellationToken token)
        {
            ...
            progress?.ReportStatus($"Downloading OdyPatch {version}...");
            ...
            using (HttpClient client = CreateHttpClient())
            {
                foreach (string mirror in mirrors)
                {
                    try
                    {
                        ...
                        string fileName = Path.GetFileName(string.IsNullOrWhiteSpace(uri.AbsolutePath) ? $"odypatch_{version}.zip" : uri.AbsolutePath);
        ...
        {
            string root = Path.Combine(Path.GetTempPath(), "odypatch_update", Guid.NewGuid().ToString("N", CultureInfo.InvariantCulture));
        ...
        private static HttpClient CreateHttpClient()
        {
            ...
            client.DefaultRequestHeaders.UserAgent.ParseAdd("OdyPatch AutoUpdater");
        ...
        private async Task ApplyUpdateAsync(string payloadRoot, IUpdateProgress progress, CancellationToken token)
        {
            ...
            string scriptPath = Path.Combine(scriptDirectory,
                IsWindows() ? $"update_{processId}.ps1" : $"update_{processId}.sh");
