...
namespace OdyTools.Update
{
    ...
    public class AppUpdate
    {
        ...
        private List<string> GetArchiveNames()
        {
            ...
            // Normalize OS name
            if (osName.IndexOf("Windows", StringComparison.OrdinalIgnoreCase) >= 0)
            {
            ...
            if (osName.IndexOf("Linux", StringComparison.OrdinalIgnoreCase) >= 0)
            {
                return new List<string>
                {
            ...
            if (osName.IndexOf("Darwin", StringComparison.OrdinalIgnoreCase) >= 0 || osName.IndexOf("macOS", StringComparison.OrdinalIgnoreCase) >= 0)
            {
                return new List<string>
                {
                ...
                foreach (string url in _updateUrls)
                {
                    try
                    {
                        ...
                        using (var httpClient = new HttpClient())
                        {
                            ...
                            httpClient.DefaultRequestHeaders.UserAgent.ParseAdd("OdyTools-Updater/1.0");
