...
2. **Remaining test gaps** — build-pipeline commands (`init`, `config`, `convert`, `compile`, `pack`, `install`, `unpack`, `list`) and format convert have automated coverage; archive list/search helpers and `list-archive`/`search-archive` error paths are tested; `launch` path resolution (`ResolveGameBinary`) has direct unit tests. **172** tests in `KotorCLI.Tests` on net9.0. `launch` remains stub-only.
