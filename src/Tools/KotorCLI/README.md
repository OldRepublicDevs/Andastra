...
2. **Remaining test gaps** — build-pipeline commands (`init`, `config`, `convert`, `compile`, `pack`, `install`, `unpack`, `list`) and format convert have automated coverage; archive create/list/search/extract helpers and error paths are tested; `list-archive` / `search-archive` on RIM, MOD (incl. verbose, filter, and case-sensitive paths), and BIF+KEY; `extract --filter` (RIM, MOD, BIF+KEY, and KEY) and `create-archive --filter` (RIM and MOD) have integration coverage; `launch` path resolution (`ResolveGameBinary`) has direct unit tests. **200** tests in `KotorCLI.Tests` on net9.0. `launch` remains stub-only.
