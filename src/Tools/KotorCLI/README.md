...
2. **Remaining test gaps** — build-pipeline commands (`init`, `config`, `convert`, `compile`, `pack`, `install`, `unpack`, `list`) and format convert have automated coverage; archive create/list/search/extract helpers and error paths are tested; `list-archive` / `search-archive` on RIM, MOD (incl. verbose, filter, content-disabled, and case-sensitive paths), and BIF+KEY; `extract` (RIM, MOD, ERF, BIF+KEY, and KEY incl. BIF-stem output) and `create-archive --filter` (RIM, MOD, and ERF) have integration coverage; `launch` path resolution (`ResolveGameBinary`) has direct unit tests. **209** tests in `KotorCLI.Tests` on net9.0. `launch` remains stub-only.
