...
namespace BioWare.Tests
{
    ...
    public class NcsConstiScannerTests
    {
        ...
        [Test]
        public void GetConstiUsageContext_FiveHopMixedConstCptopspRelayStrRef_ReturnsStrRefConsumer()
        {
            const int targetStrRef = 424242;
            NCS ncs = NCSAuto.CompileNss(
                "void speak(int a, int s) { ActionSpeakStringByStrRef(s); }\nvoid inner(int a, int s) { speak(0, s); }\nvoid mid2(int a, int s) { inner(a, s); }\nvoid mid(int a, int s) { mid2(a, s); }\nvoid relay(int a, int s) { mid(a, s); }\nvoid outer(int a, int s) { relay(a, s); }\nvoid main() { outer(99, " + targetStrRef + "); }",
                BioWareGame.K1);
            byte[] bytes = NCSAuto.BytesNcs(ncs);
        ...
        [Test]
        public void StrRefReferenceCache_FiveHopMixedConstCptopspRelayStrRef_IsIndexed()
        {
            const int targetStrRef = 424242;
            NCS ncs = NCSAuto.CompileNss(
                "void speak(int a, int s) { ActionSpeakStringByStrRef(s); }\nvoid inner(int a, int s) { speak(0, s); }\nvoid mid2(int a, int s) { inner(a, s); }\nvoid mid(int a, int s) { mid2(a, s); }\nvoid relay(int a, int s) { mid(a, s); }\nvoid outer(int a, int s) { relay(a, s); }\nvoid main() { outer(99, " + targetStrRef + "); }",
            ...
            string filepath = Path.Combine(Path.GetTempPath(), "ncs-five-hop-mixed-const-jsr-" + Guid.NewGuid().ToString("N") + ".ncs");
            File.WriteAllBytes(filepath, bytes);
