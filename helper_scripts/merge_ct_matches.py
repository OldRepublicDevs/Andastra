"""Merge tsl_cascade_translated_matches.json into cascade and regenerate rename script."""
import json, re

CASCADE = "docs/k1_tsl_cascade_matches.json"
NEW     = "docs/tsl_cascade_translated_matches.json"
...
added = 0
for m in new_:
    if m["tsl_addr"] not in existing:
        cas["matches"].append(m)
        existing.add(m["tsl_addr"])
        added += 1
print("Added {}  (total now {})".format(added, len(cas["matches"])))
